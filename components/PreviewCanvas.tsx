  ...
  useEffect(() => {
    ...
    const drawImages = async () => {
      try {
        ...
        // Draw text elements (always on top)
        textElements.forEach(textEl => {
          const fontSize = 32 * textEl.scale;
          const x = 300 + textEl.position.x; // Center of canvas
          const y = 300 + textEl.position.y;
          
          // Measure text to get bounds
          ctx.font = `${fontSize}px "Comic Sans MS", "Comic Sans", cursive`;
          ...
          if (selectedItems.includes(textEl.id)) {
            ctx.save();
            ctx.strokeStyle = '#3b82f6';
            ctx.lineWidth = 2;
            ctx.setLineDash([5, 5]);
    ...
    drawingOrder, selectedItems, textElements
  ]);
  ...
  const handleExport = () => {
    ...
    try {
      ...
      const drawWithoutSelection = async () => {
        ...
        // Draw text elements on export
        textElements.forEach(textEl => {
          const fontSize = 32 * textEl.scale;
          const x = 300 + textEl.position.x;
          const y = 300 + textEl.position.y;
          
          tempCtx.save();
          tempCtx.translate(x, y);
          tempCtx.rotate((textEl.rotation * Math.PI) / 180);
          tempCtx.fillStyle = '#000000';
          tempCtx.textAlign = 'center';
          tempCtx.textBaseline = 'middle';
          tempCtx.font = `${fontSize}px "Comic Sans MS", "Comic Sans", cursive`;
          tempCtx.fillText(textEl.text, 0, 0);
          tempCtx.restore();
        });
